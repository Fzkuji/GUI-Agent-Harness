#!/usr/bin/env python3
"""qwen enable_thinking on/off:精度 + 延迟,point2d_1000+hires,同 baseline50。
纯参数切换,同一次调用,不加调用次数。
用法: python probe_qwen_thinking.py [workers]
"""
from __future__ import annotations
import base64, io, json, re, sys, glob, time
...
def ask(path, instr, thinking):
    prompt = ("Locate the UI element to click that fulfils the instruction, and output its "
              f"center point.\nInstruction: {instr}\n" + cf.prompt_suffix("point2d_1000", 0, 0))
    ...
    for thinking in [True, False]:
        ...
        def work(sid):
            ...
            try:
                ...
                resp, dt = ask(str(IMG_DIR / f"{sid}.png"), r["instruction"], thinking)
                p = cf.parse_point(resp, "point2d_1000", W, H)
        ...
        with open(OUT / f"qwen_thinking_{tag}.jsonl", "w", encoding="utf-8") as o:
            for rr in recs:
        ...
        print(f"[thinking={tag:3s}] {ok}/{len(done)} = {ok/max(1,len(done)):.0%}  "
