#!/usr/bin/env python3
"""MiniMax-M3 探针B/C:hints on/off,在母语格式(point2d_1000)和非母语格式(abs_pixel)各测一次。
诊断目标:证据注入效应的符号是否随格式反转(qwen 的专训头signature)。
同 probe_kimi_hints_reversal.py 的方法论,但走 openprogram Runtime.exec()
(provider=minimax-cn-coding-plan),与 run_sspro_native.py 的 M3 全量跑法完全一致的调用路径。
用法: python probe_m3_hints_reversal.py [workers]
"""
...
# point2d_1000 是 M3 的母语格式(27%,消融最优),abs_pixel 是它最差的格式(0%)
CONDITIONS = [("point2d_1000", "native"), ("abs_pixel", "foreign")]
...
def main():
    ...
    for fmt, tag in CONDITIONS:
        for use_hint in (False, True):
            ...
            out_path = OUT / f"m3_reversal_{key}.jsonl"
            done = set()
            if out_path.exists():
            ...
            print(f"[M3 {key:22s}] {ok}/{len(done_r)} = {ok/max(1,len(done_r)):.0%}", flush=True)
    ...
    for tag in ("native", "foreign"):
        ...
        print(f"{tag:8s}({fmt:12s}): 无提示 {no_ok}/{no_n} -> 有提示 {hi_ok}/{hi_n}  "
