#!/usr/bin/env python3
"""Summarize UI-Vision / MMBench-GUI-L2 evaluation progress."""
...
def read_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    for line_no, line in enumerate(path.read_text(errors="replace").splitlines(), 1):
        ...
        try:
            ...
            print(f"[gui-grounding-report] skip invalid JSONL {path}:{line_no}")
    return rows
...
def load_plan(run_dir: Path) -> list[dict[str, Any]]:
    ...
    for line in plan_path.read_text().splitlines():
        if not line.strip():
            continue
        dataset, annotation, index, sample_id, split, shard = line.split("\t")[:6]
        entries.append(
            {
                "dataset": dataset,
...
def format_report(payload: dict[str, Any], alive: bool | None) -> str:
    ...
    lines = [f"GUI grounding {dataset_names}:"]
