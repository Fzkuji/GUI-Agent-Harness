#!/usr/bin/env python3
"""Qwen 官方原生 grounding 格式测试:归一化 [0,1000] + point_2d + hires。
对比我们之前的 [0,1] 分数格式(58%),隔离"训练时坐标格式"这个变量。
用法: python probe_qwen_official.py [workers]
"""
...
# 官方 Qwen3-VL grounding:point_2d + [0,1000] 归一化整数
...
def main():
    ...
    with open(OUT / "qwen_official.jsonl", "w", encoding="utf-8") as o:
        for rr in recs:
            o.write(json.dumps(rr, ensure_ascii=False) + "\n")
    done = [x for x in recs if "hit" in x]
    ok = sum(x["hit"] for x in done)
    print(f"[官方格式 point_2d/[0,1000]+hires] {ok}/{len(done)} = {ok/max(1,len(done)):.0%} "
          f"(+{len(recs)-len(done)}err)")
    print("对比: 我们[0,1]格式 default=52% / hires=58%")
    print("前5条原始:", raw[:5])
